...
import {
    ...
    RiCheckboxMultipleLine, RiArrowUpWideLine, RiArrowDownWideLine,
                ...
                {mediaInfo?.details &&
                    <div className="w-full h-full flex flex-col gap-12">
                        ...
                        <div className="w-full h-full flex flex-col gap-12 items-stretch 880:flex-row">
                            {/* Part 1 : Media Poster */}
                            <div className="hidden w-full flex-col flex-shrink-0 880:flex 880:w-[18rem]">
                            ...
                            <div className="flex flex-col gap-4 max-w-2xl">
                                <h1 className="text-3xl font-medium sm:text-4xl">{mediaInfo?.details?.name || "N/A"}</h1>
                                <div className="flex items-center flex-wrap text-sm gap-2">
                                    ...
                                    {(mediaInfo?.details?.first_air_date || mediaInfo?.details?.origin_country?.[0]) &&
                                        <div className="flex gap-1">
                                            <span>{mediaInfo?.details?.first_air_date?.slice(0, 4)}</span>
                                            {mediaInfo?.details?.origin_country?.[0] && <span>({mediaInfo?.details?.origin_country?.[0]})</span>}
                                        </div>
                                    }
                                    ...
                                    <div className="w-full flex gap-4 flex-wrap 295:flex-nowrap">
                                        ...
                                        <div onClick={() => saveUsersMedia(mediaInfo?.details, 'favourite')} className="w-full flex justify-center items-center gap-2 border-1 border-textcolor-secondary px-3 py-2 rounded cursor-pointer active:scale-[0.95]">
                                            {(showSavedUsersMedia(mediaInfo?.details, 'favourite')) ?
                                                <RiHeartFill className='w-[1.80rem] h-[1.80rem] text-[#f14049]' />
                                                :
                                                <RiHeartLine className='w-[1.80rem] h-[1.80rem] text-[#A9A9A9]' />
                                            }
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="w-full flex flex-col gap-5 880:flex-row 880:gap-10">
                            ...
                            {(mediaInfo?.details?.first_air_date) &&
                                <div className="w-full flex gap-2 p-4 bg-black/40 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] 880:justify-center">
                                    <div className="flex justify-center items-center">
                                        <RiCalendarEventLine className="w-7 h-7" />
                                    </div>
                                    <div className="flex flex-col">
                                        <h1 className="text-sm font-medium whitespace-nowrap">Release Date</h1>
                            ...
                            <div className="w-full flex flex-col gap-5">
                                ...
                                <div className="grid grid-flow-col auto-cols-[10rem] gap-4 overflow-x-auto no-scrollbar items-stretch">
                                    {mediaInfo?.credits?.cast?.slice(0, 20)?.map((cast) => {
                                        const castRoles = (cast?.roles?.length > 0 ? cast?.roles?.slice(0, 1)?.map((roles) => roles?.character)?.join(", ") : null);
                            ...
                            <div className="w-full flex flex-col justify-around gap-5 rounded-lg cursor-pointer">
                                ...
                                {(sectionOneType === "Networks") &&
                                    (mediaInfo?.details?.networks?.length > 0 ?
                                        (<div className="flex flex-row gap-5">
                                            <div className="w-full bg-black/40 flex flex-col gap-8 p-8 pt-12 rounded-md" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                                <h1 className="text-base font-semibold underline">The Networks Behind <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span></h1>
                                                <div className="w-full max-h-70 overflow-y-auto custom-scrollbar flex flex-col gap-6 pr-6 cursor-pointer">
                                                    {mediaInfo?.details?.networks?.map((network) => {
                                                        return (
                                                            <div key={network.id} onClick={() => (network?.name && mediaInfo?.details?.name) && window.open(`https://www.google.com/search?q=${network?.name}+Networks+${mediaInfo?.details?.name}`, '_blank')} className="w-full flex flex-col gap-2 items-start 460:items-center 460:flex-row">
                                                                ...
                                                                {network?.name &&
                                                                    <div className="flex flex-col gap-1">
                                                                        {network?.name && <h1 className="text-sm font-semibold">— {network?.name} {network?.origin_country && <span className="text-sm">{`(${network?.origin_country})`}</span>}</h1>}
                                                                    </div>
                                                                }
                                                            </div>
                                                        )
                                        ...
                                        (
                                            <div className="w-full text-center py-20 px-10 bg-black/40 rounded-sm">
                                                <h1 className="text-base font-semibold sm:text-base">Network information unavailable</h1>
                                            </div>
                                        )
                                    )}
                                ...
                                {(sectionOneType === "Seasons") &&
                                    (mediaInfo?.details?.seasons?.length > 0 ?
                                        (<div className="flex flex-row gap-5" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                            <div className="w-full bg-black/40 flex flex-col gap-8 py-8 px-3 pt-12 rounded-md 430:px-8 430:py-8 430:pt-12">
                                                <h1 className="text-base font-semibold underline">All seasons of <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span></h1>
                                                            ...
                                                            <div key={season.id} className="flex flex-col gap-5 border-[0.1px] border-white/30 rounded-2xl">
                                                                ...
                                                                <div className="w-full flex flex-col gap-5 p-5">
                                                                    <div className="w-full h-full flex flex-col gap-8 700:flex-row">
                                                                        {/* Season's poster */}
                                                                        <div className="relative w-full overflow-hidden rounded-sm flex-shrink-0 700:w-[14rem]">
                                                                            <img src={(season?.poster_path || mediaInfo?.details?.poster_path) ? `${IMG_POSTER_BASE_URL}${(season?.poster_path || mediaInfo?.details?.poster_path)}` : NoPoster} alt="SeasonPoster" className="w-full aspect-[1/1] object-cover object-top 700:h-full 700:aspect-auto" />
                                                                        </div>
                                                                        ...
                                                                        <div className="flex flex-col gap-4 max-w-2xl">
                                                                            ...
                                                                            {(season?.air_date) &&
                                                                                <div className="flex text-sm">
                                                                                    <h2 className="text-xs text-textcolor-secondary border-[0.5px] rounded-2xl px-[10px] py-[2px]"><span className="hidden 295:inline-block">Premiered : </span> {season?.air_date?.replaceAll('-', '/')}</h2>
                                                                                </div>
                                                                            }
                                                                            ...
                                                                            {(season?.vote_average !== null) &&
                                                                                <div className="flex gap-4 items-center text-base">
                                                                                    {season?.vote_average !== null && <h3 className="font-medium text-base"><span className="text-yellow-400">★</span> {(season?.vote_average)?.toFixed(1) || "N/A"}</h3>}
                                                                                </div>
                                                                            }
                                                                            ...
                                                                            {(season?.season_number !== null || season?.episode_count !== null) &&
                                                                                <div className="flex gap-2 items-center text-sm flex-wrap">
                                                                                    {season?.season_number !== null && <h3 className='text-xs px-2 py-[0.10rem] rounded-4xl bg-white/10 backdrop-blur-md border border-white/20 text-white text-nowrap'>Season : {season?.season_number || "N/A"}</h3>}
                                                                                    {season?.episode_count !== null && <h3 className='text-xs px-2 py-[0.10rem] rounded-4xl bg-white/10 backdrop-blur-md border border-white/20 text-white text-nowrap'>Episodes : {season?.episode_count || "N/A"}</h3>}
                                                                                </div>
                                                                            }

                                                                            {/* If season's poster unavailable */}
                                                                            {season?.poster_path === null && <h4 className="italic text-textcolor-secondary text-sm">(Season's poster unavailable)</h4>}
                                        ...
                                        (
                                            <div className="w-full text-center py-20 px-10 bg-black/40 rounded-sm">
                                                <h1 className="text-base font-semibold sm:text-base">Seasons's information unavailable</h1>
                                            </div>
                                        )
                                    )}
                                ...
                                {(sectionOneType === "AiringStatus") &&
                                    ((mediaInfo?.details?.last_episode_to_air || mediaInfo?.details?.next_episode_to_air) !== null ?
                                            ...
                                            <div className="w-full bg-black/40 py-8 px-3 pt-18 rounded-md overflow-y-auto custom-scrollbar 430:px-8 430:py-8 430:pt-18">
                                                <div className="w-full flex flex-col py-2 gap-8 max-h-110 pr-3 overflow-y-auto custom-scrollbar 430:pr-5">
                                                    ...
                                                    {mediaInfo?.details?.last_episode_to_air !== null ?
                                                        <div className="flex flex-col gap-5 border-[0.1px] border-white/30 rounded-2xl">
                                                            ...
                                                            <div className="w-full flex flex-col gap-5 cursor-pointer p-5">
                                                                <div className="w-full h-full flex flex-col gap-8 840:flex-row">
                                                                    {/* Episode's poster */}
                                                                    <div className="relative w-full aspect-video overflow-hidden rounded-sm flex-shrink-0 840:w-[20rem]">
                                                                    ...
                                                                    <div className="flex flex-col gap-4 max-w-2xl">
                                                                        ...
                                                                        <div className="flex items-center gap-2 flex-wrap text-sm">
                                                                            {/* Episode's air date */}
                                                                            {(mediaInfo?.details?.last_episode_to_air?.air_date) && <h2>({mediaInfo?.details?.last_episode_to_air?.air_date?.replaceAll('-', '/')})</h2>}

                                                                            {/* Episode's type */}
                                                                            {(mediaInfo?.details?.last_episode_to_air?.episode_type) &&
                                                                                <div className="flex">
                                                                                    <h2>• {(mediaInfo?.details?.last_episode_to_air?.episode_type)?.charAt(0)?.toUpperCase() + mediaInfo?.details?.last_episode_to_air?.episode_type?.slice(1)}</h2>
                                                                                </div>
                                                                            }
                                                                            ...
                                                                            {(mediaInfo?.details?.last_episode_to_air?.runtime !== 0 && mediaInfo?.details?.last_episode_to_air?.runtime) &&
                                                                                <div className="flex">
                                                                                    <h2>• {`${(Math.floor(mediaInfo?.details?.last_episode_to_air?.runtime / 60)) === 0 ? "" : Math.floor(mediaInfo?.details?.last_episode_to_air?.runtime / 60)}${(Math.floor(mediaInfo?.details?.last_episode_to_air?.runtime / 60)) === 0 ? "" : "h"} ${(mediaInfo?.details?.last_episode_to_air?.runtime % 60)?.toString()?.padStart(2, '0')}m`}</h2>
                                                                                </div>
                                                                            }
                                                                        </div>
                                                                        ...
                                                                        {(mediaInfo?.details?.last_episode_to_air?.vote_average !== null || mediaInfo?.details?.last_episode_to_air?.vote_count !== null) &&
                                                                            <div className="flex gap-2 items-center text-base flex-wrap">
                                                                                {mediaInfo?.details?.last_episode_to_air?.vote_average !== null && <h3 className="font-medium text-nowrap"><span className="text-yellow-400">★</span> {(mediaInfo?.details?.last_episode_to_air?.vote_average)?.toFixed(1) || "N/A"}</h3>}
                                                                                {mediaInfo?.details?.last_episode_to_air?.vote_count !== null && <h3 className="text-sm text-nowrap">({`${mediaInfo?.details?.last_episode_to_air?.vote_count} Votes`})</h3>}
                                                                            </div>
                                                                        }
                                                                        ...
                                                                        {(mediaInfo?.details?.last_episode_to_air?.season_number !== null || mediaInfo?.details?.last_episode_to_air?.episode_number !== null) &&
                                                                            <div className="flex gap-4 items-center text-base">
                                                                                {mediaInfo?.details?.last_episode_to_air?.season_number !== null && <h3 className='text-sm px-2 py-[0.10rem] rounded-4xl bg-white/10 backdrop-blur-md border border-white/20 text-white text-nowrap'>S{mediaInfo?.details?.last_episode_to_air?.season_number || "N/A"} {mediaInfo?.details?.last_episode_to_air?.episode_number !== null && <span> • E{mediaInfo?.details?.last_episode_to_air?.episode_number || "N/A"}</span>}</h3>}
                                                                            </div>
                                                                        }

                                                                        {/* If episode's poster unavailable */}
                                                                        {mediaInfo?.details?.last_episode_to_air?.still_path === null && <h4 className="italic text-textcolor-secondary text-sm">(Episode's poster unavailable)</h4>}

                                                                        {/* Episodes's overview */}
                                                                        {mediaInfo?.details?.last_episode_to_air?.overview &&
                                                                            <div className="flex flex-col gap-2">
                                                                                <h1 className="text-base font-medium">Overview</h1>
                                                                                <p className="text-sm">{mediaInfo?.details?.last_episode_to_air?.overview}</p>
                                                                            </div>
                                                                        }
                                                                    </div>
                                                                </div>
                                                        ...
                                                        :
                                                        <div className="w-full text-center py-20 px-10 bg-black/40 rounded-sm border-[0.1px] border-white/30">
                                                            <h1 className="text-base font-semibold sm:text-base">Last episode information unavailable</h1>
                                                        </div>
                                                    }
                                                    {/* Next episode to air */}
                                                    {mediaInfo?.details?.next_episode_to_air !== null ?
                                                            ...
                                                            <div className="w-full flex flex-col gap-5 cursor-pointer p-5">
                                                                <div className="w-full h-full flex flex-col gap-8 840:flex-row">
                                                                    ...
                                                                    <div className="flex flex-col gap-4 max-w-2xl">
                                                                        ...
                                                                        <div className="flex items-center gap-2 flex-wrap text-sm">
                                                                            {/* Episode's air date */}
                                                                            {(mediaInfo?.details?.next_episode_to_air?.air_date) && <h2>({mediaInfo?.details?.next_episode_to_air?.air_date?.replaceAll('-', '/')})</h2>}

                                                                            {/* Episode's type */}
                                                                            {(mediaInfo?.details?.next_episode_to_air?.episode_type) &&
                                                                                <div className="flex">
                                                                                    <h2>• {(mediaInfo?.details?.next_episode_to_air?.episode_type)?.charAt(0)?.toUpperCase() + mediaInfo?.details?.next_episode_to_air?.episode_type?.slice(1)}</h2>
                                                                                </div>
                                                                            }
                                                                            ...
                                                                            {(mediaInfo?.details?.next_episode_to_air?.runtime !== 0 && mediaInfo?.details?.next_episode_to_air?.runtime) &&
                                                                                <div className="flex">
                                                                                    <h2>• {`${(Math.floor(mediaInfo?.details?.next_episode_to_air?.runtime / 60)) === 0 ? "" : Math.floor(mediaInfo?.details?.next_episode_to_air?.runtime / 60)}${(Math.floor(mediaInfo?.details?.next_episode_to_air?.runtime / 60)) === 0 ? "" : "h"} ${(mediaInfo?.details?.next_episode_to_air?.runtime % 60)?.toString()?.padStart(2, '0')}m`}</h2>
                                                                                </div>
                                                                            }
                                                                        </div>
                                                                        ...
                                                                        {(mediaInfo?.details?.next_episode_to_air?.vote_average !== null || mediaInfo?.details?.next_episode_to_air?.vote_count !== null) &&
                                                                            <div className="flex gap-2 items-center text-base flex-wrap">
                                                                                {mediaInfo?.details?.next_episode_to_air?.vote_average !== null && <h3 className="font-medium text-nowrap"><span className="text-yellow-400">★</span> {(mediaInfo?.details?.next_episode_to_air?.vote_average)?.toFixed(1) || "N/A"}</h3>}
                                                                                {mediaInfo?.details?.next_episode_to_air?.vote_count !== null && <h3 className="text-sm text-nowrap">({`${mediaInfo?.details?.next_episode_to_air?.vote_count} Votes`})</h3>}
                                                                            </div>
                                                                        }
                                                                        ...
                                                                        {(mediaInfo?.details?.next_episode_to_air?.season_number !== null || mediaInfo?.details?.next_episode_to_air?.episode_number !== null) &&
                                                                            <div className="flex gap-4 items-center text-base">
                                                                                {mediaInfo?.details?.next_episode_to_air?.season_number !== null && <h3 className='text-sm px-2 py-[0.10rem] rounded-4xl bg-white/10 backdrop-blur-md border border-white/20 text-white text-nowrap'>S{mediaInfo?.details?.next_episode_to_air?.season_number || "N/A"} {mediaInfo?.details?.next_episode_to_air?.episode_number !== null && <span> • E{mediaInfo?.details?.next_episode_to_air?.episode_number || "N/A"}</span>}</h3>}
                                                                            </div>
                                                                        }

                                                                        {/* If episode's poster unavailable */}
                                                                        {mediaInfo?.details?.next_episode_to_air?.still_path === null && <h4 className="italic text-textcolor-secondary text-sm">(Episode's poster unavailable)</h4>}

                                                                        {/* Episodes's overview */}
                                                                        {mediaInfo?.details?.next_episode_to_air?.overview &&
                                                                            <div className="flex flex-col gap-2">
                                                                                <h1 className="text-base font-medium">Overview</h1>
                                                                                <p className="text-sm">{mediaInfo?.details?.next_episode_to_air?.overview}</p>
                                                                            </div>
                                                                        }
                                                                    </div>
                                                                </div>
                                                        ...
                                                        :
                                                        <div className="w-full text-center py-20 px-10 bg-black/40 rounded-sm border-[0.1px] border-white/30">
                                                            <h1 className="text-base font-semibold sm:text-base">No upcoming episode available</h1>
                                                        </div>
                                                    }
                                                </div>
                                            </div>
                                        ...
                                        (
                                            <div className="w-full text-center py-20 px-10 bg-black/40 rounded-sm">
                                                <h1 className="text-base font-semibold sm:text-base">Airing information unavailable</h1>
                                            </div>
                                        )
                                    )}
                            ...
                            <div className="w-full flex flex-col justify-around gap-5 rounded-lg cursor-pointer">
                                    ...
                                    (mediaInfo?.reviews?.results?.length > 0 ?
                                        (<div className="flex flex-row gap-5 no-scrollbar">
                                            <div className="w-full bg-black/40 flex flex-col gap-6 py-8 px-3 pt-12 rounded-md 430:px-8 430:py-8 430:pt-12" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                                <h1 className="text-base font-semibold underline">Audience Reactions on <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span></h1>
                                ...
                                {(sectionTwoType === "Studios") &&
                                    (mediaInfo?.details?.production_companies?.length > 0 ?
                                        (<div className="flex flex-row gap-5">
                                            <div className="w-full bg-black/40 flex flex-col gap-8 p-8 pt-12 rounded-md" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                                <h1 className="text-base font-semibold underline">The Studios Behind <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span></h1>
                                                <div className="w-full max-h-70 overflow-y-auto custom-scrollbar flex flex-col gap-6 pr-6 cursor-pointer">
                                                    {mediaInfo?.details?.production_companies?.map((studio) => {
                                                        return (
                                                            <div key={studio.id} onClick={() => studio?.name && window.open(`https://www.google.com/search?q=${studio?.name}+Production+Company`, '_blank')} className="w-full flex flex-col gap-2 items-start 460:items-center 460:flex-row">
                                                                ...
                                                                {studio?.name &&
                                                                    <div className="flex flex-col gap-1">
                                                                        {studio?.name && <h1 className="text-sm font-semibold">— {studio?.name} {studio?.origin_country && <span className="text-sm">{`(${studio?.origin_country})`}</span>}</h1>}
                                                                    </div>
                                                                }
                                                            </div>
                                                        )
                                ...
                                {(sectionTwoType === "WatchProviders") &&
                                    (regionalWatchProviderType?.length > 0 ?
                                        (<div className="flex flex-row gap-5 no-scrollbar">
                                            <div className="w-full bg-black/40 flex flex-col gap-6 py-8 px-3 pt-12 rounded-md 430:px-8 430:py-8 430:pt-12" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                                <h1 className="text-base font-semibold underline">Streaming Options for <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span> available on</h1>
                                                <div className="w-full max-h-100 overflow-y-auto custom-scrollbar flex flex-col gap-6 pr-3 cursor-pointer 430:pr-5">
                                                    {/* Buy */}
                                                    {(regionalWatchProvider?.watchProviders?.buy?.length) > 0 &&
                                                        <div className="w-full bg-black/40 flex flex-col gap-5 p-8 rounded-md">
                                                            <h1 className="text-base font-semibold underline">Buy to own</h1>
                                                    ...
                                                    {(regionalWatchProvider?.watchProviders?.flatrate?.length) > 0 &&
                                                        <div className="w-full bg-black/40 flex flex-col gap-5 p-8 rounded-md">
                                                            <h1 className="text-base font-semibold underline">Included with subscription</h1>
                                                            <div className="flex flex-col gap-5 cursor-pointer">
                                                    ...
                                                    {(regionalWatchProvider?.watchProviders?.rent?.length) > 0 &&
                                                        <div className="w-full bg-black/40 flex flex-col gap-5 p-8 rounded-md">
                                                            <h1 className="text-base font-semibold underline">Rent & watch</h1>
                                                            <div className="flex flex-col gap-5 cursor-pointer">
                                                    ...
                                                    {(regionalWatchProvider?.watchProviders?.ads?.length) > 0 &&
                                                        <div className="w-full bg-black/40 flex flex-col gap-5 p-8 rounded-md">
                                                            <h1 className="text-base font-semibold underline">Watch with ads</h1>
                                                            <div className="flex flex-col gap-5 cursor-pointer">
                                                    ...
                                                    {(regionalWatchProvider?.watchProviders?.free?.length) > 0 &&
                                                        <div className="w-full bg-black/40 flex flex-col gap-5 p-8 rounded-md">
                                                            <h1 className="text-base font-semibold underline">Stream for free</h1>
                                                            <div className="flex flex-col gap-5 cursor-pointer">
                                ...
                                {(sectionTwoType === "Socials") &&
                                    (mediasSocialsCheck !== null ?
                                        (<div className="flex flex-row gap-5">
                                            <div className="w-full bg-black/40 flex flex-col gap-8 p-8 pt-12 rounded-md" style={{ clipPath: 'polygon(calc(100% - 40px) 20px, 100% 0, 100% 100%, 0 100%, 0 20px)' }}>
                                                <h1 className="text-base font-semibold underline">Follow <span className="italic">{mediaInfo?.details?.name ? mediaInfo?.details?.name : "this TV Show"}</span> on Social Media</h1>
                                                <div className="w-full grid grid-cols-[repeat(auto-fit,minmax(100px,1fr))] sm:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-x-5 gap-y-6 mx-auto cursor-pointer">
                                                    ...
                                                    {(mediaInfo?.external_ids?.tvdb_id) &&
                                                        <div onClick={() => mediaInfo?.external_ids?.tvdb_id && window.open(`https://thetvdb.com/?id=/${mediaInfo?.external_ids?.tvdb_id}&tab=series`, '_blank')} className="flex gap-2 items-center">
                                                            <RiSlideshow4Line className="w-8 h-8" />
                                                            <h1 className="text-sm font-normal 430:text-base">TVDB</h1>
                                                        </div>
                                                    }
                        ...
                        {(mediaInfo?.recommendations?.results?.length > 0) &&
                            < div className="w-full flex flex-col gap-5">
                                <div className="font-medium text-xl">
                                    <h1>If you liked <span className="italic text-textcolor-secondary">{(mediaInfo?.details?.name || "this TV Show")?.replace(".", "")} </span>, you might also like</h1>
                                </div>
                                <div className="flex flex-row gap-4 no-scrollbar overflow-x-scroll">
