...
const Movies = () => {
  ...
  const { moviesCat, allGenres, moviesBGVideo } = useSelector(
    (store) => store.content,
  );
          ...
          <div className="relative z-10 w-full min-h-[75dvh] flex flex-col justify-between gap-10 navPadding">
            ...
            {moviesGenre?.length > 0 ? (
              ...
              <div className="relative z-20 flex flex-row items-end gap-6 flex-wrap">
                ...
                >
                  ...
                  <svg
                    className={`w-3 transition-transform duration-300 ${isMoviesGenreClicked ? "rotate-180" : "rotate-0"}`}
                    viewBox="8 10 8 4"
                    fill="currentColor"
                  >
            ...
            <div className="max-w-md flex flex-col gap-4">
              <h1 className="text-3xl lg:text-4xl font-bold">
                {moviesBGVideo?.video?.title || moviesBGVideo?.video?.name}
              </h1>
              {moviesBGVideo?.video?.genre_ids?.length !== 0 && (
                <div className="flex flex-row gap-2">
                  {allGenres
                    ?.filter((list) =>
                      moviesBGVideo?.video?.genre_ids?.includes(list?.id),
                    )
                    ?.slice(0, 2)
                    ?.map((val) => (
                      <h3
