                    ...
                    return (
                        <div key={categorie.type} className="w-full flex flex-col gap-3">
                            ...
                            <div className="flex flex-row gap-4 overflow-x-scroll no-scrollbar cursor-pointer">
                                {(categorie.content.filter((content) => content.id && content.poster_path)).map((content, index) => (
                ...
                {(popularCat.filter((categorie) => categorie.content.length > 5)).slice(1, 2).map((categorie) => {
                    return categorie.title === "Top Popular Movies" && (
                        <div key={categorie.type} className="w-full flex flex-col gap-3">
                            ...
                            <div className="flex flex-row gap-4 overflow-x-scroll no-scrollbar cursor-pointer">
                                {/* 1-9 */}
                                {(categorie.content.filter((content) => content.id && content.poster_path)).slice(0, 9).map((content, index) => (
                                    <div key={content.id} onClick={() => mediaType(content)} className="relative w-[16rem] h-[12rem] md:w-[18rem] md:h-[14rem] flex-shrink-0 group">
                                        <div className="relative flex justify-center items-center rounded-sm overflow-hidden w-full h-full transition-transform duration-300 ease-out group-hover:scale-95">
                                            <img src={`${IMG_POSTER_BASE_URL}${content.poster_path}`} alt="Poster" className="absolute right-0 z-10 w-[67%] md:w-[65%] h-full object-cover" />
                                ...
                                {/* 10-15 */}
                                {(categorie.content.filter((content) => content.id && content.poster_path)).slice(9, 15).map((content, index) => (
                                    <div key={content.id} onClick={() => mediaType(content)} className="relative w-[23rem] h-[12rem] md:w-[25rem] md:h-[14rem] flex-shrink-0 group">
                                        <div className="relative flex justify-center items-center rounded-sm overflow-hidden w-full h-full transition-transform duration-300 ease-out group-hover:scale-95">
                                            <img src={`${IMG_POSTER_BASE_URL}${content.poster_path}`} alt="Poster" className="absolute right-0 z-10 w-[58%] md:w-[50%] h-full object-cover" />
