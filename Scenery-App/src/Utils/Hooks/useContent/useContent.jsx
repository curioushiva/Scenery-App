...
const useContent = () => {
    ...
    try {
      const responses = await Promise.allSettled(
        landingContentData.map((content) =>
          axios.get(`${SCENERY_API_MEDIA_BASE_URL}${content.url}`),
        ),
      );
  ...
  const getAllGenres = async () => {
    try {
      const responses = await Promise.allSettled(
        genreData.map((val) =>
          axios.get(`${SCENERY_API_MEDIA_BASE_URL}${val.url}`),
        ),
      );
  ...
  const getBackgroundVideo = async (getVideoList) => {
    ...
    try {
      ...
      const responses = await axios.get(
        `${SCENERY_API_MEDIA_BASE_URL}/${mediaType}/${validBackgroundVideo.id}/videos`
      );
    ...
    try {
      ...
      const responses = await Promise.allSettled(
        browseCatData.map((cat) =>
          axios.get(`${SCENERY_API_MEDIA_BASE_URL}${cat.url}`),
        ),
      );
    ...
    try {
      ...
      const responses = await Promise.allSettled(
        movieInfoData.map((val) =>
          axios.get(`${SCENERY_API_MEDIA_BASE_URL}/movie/${mediaID}${val.url}`),
        ),
      );
    ...
    try {
      ...
      const responses = await Promise.allSettled(
        tvShowInfoData.map((val) =>
          axios.get(`${SCENERY_API_MEDIA_BASE_URL}/tv/${mediaID}${val.url}`),
        ),
      );
