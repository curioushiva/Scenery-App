import { SCENERY_API_MEDIA_BASE_URL, SCENERY_API_AI_BASE_URL } from "@/Utils/SceneryAPI/SceneryAPI";
  ...
  const getSearch = async (query) => {
    try {
      const response = await axios.get(
        `${SCENERY_API_MEDIA_BASE_URL}/search/multi?query=${query}`,
      );
  ...
  const getAskAI = async (query) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${SCENERY_API_AI_BASE_URL}/chat/completions`,
        body,
      );
      ...
      const responses = await Promise.allSettled(
        aiRecommendation.map((recommendation) =>
          recommendation?.type === "movie"
            ? axios.get(
              `${SCENERY_API_MEDIA_BASE_URL}/search/movie?query=${encodeURIComponent(
                recommendation?.title,
              )}&year=${recommendation?.year}`,
            )
            : axios.get(
              `${SCENERY_API_MEDIA_BASE_URL}/search/tv?query=${encodeURIComponent(
                recommendation?.title,
